// Code generated by protoc-gen-ts_proto. DO NOT EDIT.
// versions:
//   protoc-gen-ts_proto  v2.12.0
//   protoc               v3.19.1
// source: core/v1/runtime.proto
/* eslint-disable */
import { BinaryReader, BinaryWriter } from "@bufbuild/protobuf/wire";
import { makeGenericClientConstructor, } from "@grpc/grpc-js";
import { DeleteResponse, EnvVar, Timestamps } from "../../common/v1/types.js";
export const protobufPackage = "core.v1";
function createBaseRuntime() {
    return {
        id: "",
        name: "",
        description: "",
        providerId: "",
        allowedModels: [],
        defaultModel: "",
        containerImage: "",
        installCommand: "",
        envVars: [],
        agent: "",
        timestamps: undefined,
        dockerfile: "",
    };
}
export const Runtime = {
    encode(message, writer = new BinaryWriter()) {
        if (message.id !== "") {
            writer.uint32(10).string(message.id);
        }
        if (message.name !== "") {
            writer.uint32(18).string(message.name);
        }
        if (message.description !== "") {
            writer.uint32(26).string(message.description);
        }
        if (message.providerId !== "") {
            writer.uint32(34).string(message.providerId);
        }
        for (const v of message.allowedModels) {
            writer.uint32(42).string(v);
        }
        if (message.defaultModel !== "") {
            writer.uint32(50).string(message.defaultModel);
        }
        if (message.containerImage !== "") {
            writer.uint32(58).string(message.containerImage);
        }
        if (message.installCommand !== "") {
            writer.uint32(66).string(message.installCommand);
        }
        for (const v of message.envVars) {
            EnvVar.encode(v, writer.uint32(74).fork()).join();
        }
        if (message.agent !== "") {
            writer.uint32(82).string(message.agent);
        }
        if (message.timestamps !== undefined) {
            Timestamps.encode(message.timestamps, writer.uint32(90).fork()).join();
        }
        if (message.dockerfile !== "") {
            writer.uint32(98).string(message.dockerfile);
        }
        return writer;
    },
    decode(input, length) {
        const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
        const end = length === undefined ? reader.len : reader.pos + length;
        const message = createBaseRuntime();
        while (reader.pos < end) {
            const tag = reader.uint32();
            switch (tag >>> 3) {
                case 1: {
                    if (tag !== 10) {
                        break;
                    }
                    message.id = reader.string();
                    continue;
                }
                case 2: {
                    if (tag !== 18) {
                        break;
                    }
                    message.name = reader.string();
                    continue;
                }
                case 3: {
                    if (tag !== 26) {
                        break;
                    }
                    message.description = reader.string();
                    continue;
                }
                case 4: {
                    if (tag !== 34) {
                        break;
                    }
                    message.providerId = reader.string();
                    continue;
                }
                case 5: {
                    if (tag !== 42) {
                        break;
                    }
                    message.allowedModels.push(reader.string());
                    continue;
                }
                case 6: {
                    if (tag !== 50) {
                        break;
                    }
                    message.defaultModel = reader.string();
                    continue;
                }
                case 7: {
                    if (tag !== 58) {
                        break;
                    }
                    message.containerImage = reader.string();
                    continue;
                }
                case 8: {
                    if (tag !== 66) {
                        break;
                    }
                    message.installCommand = reader.string();
                    continue;
                }
                case 9: {
                    if (tag !== 74) {
                        break;
                    }
                    message.envVars.push(EnvVar.decode(reader, reader.uint32()));
                    continue;
                }
                case 10: {
                    if (tag !== 82) {
                        break;
                    }
                    message.agent = reader.string();
                    continue;
                }
                case 11: {
                    if (tag !== 90) {
                        break;
                    }
                    message.timestamps = Timestamps.decode(reader, reader.uint32());
                    continue;
                }
                case 12: {
                    if (tag !== 98) {
                        break;
                    }
                    message.dockerfile = reader.string();
                    continue;
                }
            }
            if ((tag & 7) === 4 || tag === 0) {
                break;
            }
            reader.skip(tag & 7);
        }
        return message;
    },
    fromJSON(object) {
        return {
            id: isSet(object.id) ? globalThis.String(object.id) : "",
            name: isSet(object.name) ? globalThis.String(object.name) : "",
            description: isSet(object.description) ? globalThis.String(object.description) : "",
            providerId: isSet(object.providerId)
                ? globalThis.String(object.providerId)
                : isSet(object.provider_id)
                    ? globalThis.String(object.provider_id)
                    : "",
            allowedModels: globalThis.Array.isArray(object?.allowedModels)
                ? object.allowedModels.map((e) => globalThis.String(e))
                : globalThis.Array.isArray(object?.allowed_models)
                    ? object.allowed_models.map((e) => globalThis.String(e))
                    : [],
            defaultModel: isSet(object.defaultModel)
                ? globalThis.String(object.defaultModel)
                : isSet(object.default_model)
                    ? globalThis.String(object.default_model)
                    : "",
            containerImage: isSet(object.containerImage)
                ? globalThis.String(object.containerImage)
                : isSet(object.container_image)
                    ? globalThis.String(object.container_image)
                    : "",
            installCommand: isSet(object.installCommand)
                ? globalThis.String(object.installCommand)
                : isSet(object.install_command)
                    ? globalThis.String(object.install_command)
                    : "",
            envVars: globalThis.Array.isArray(object?.envVars)
                ? object.envVars.map((e) => EnvVar.fromJSON(e))
                : globalThis.Array.isArray(object?.env_vars)
                    ? object.env_vars.map((e) => EnvVar.fromJSON(e))
                    : [],
            agent: isSet(object.agent) ? globalThis.String(object.agent) : "",
            timestamps: isSet(object.timestamps) ? Timestamps.fromJSON(object.timestamps) : undefined,
            dockerfile: isSet(object.dockerfile) ? globalThis.String(object.dockerfile) : "",
        };
    },
    toJSON(message) {
        const obj = {};
        if (message.id !== "") {
            obj.id = message.id;
        }
        if (message.name !== "") {
            obj.name = message.name;
        }
        if (message.description !== "") {
            obj.description = message.description;
        }
        if (message.providerId !== "") {
            obj.providerId = message.providerId;
        }
        if (message.allowedModels?.length) {
            obj.allowedModels = message.allowedModels;
        }
        if (message.defaultModel !== "") {
            obj.defaultModel = message.defaultModel;
        }
        if (message.containerImage !== "") {
            obj.containerImage = message.containerImage;
        }
        if (message.installCommand !== "") {
            obj.installCommand = message.installCommand;
        }
        if (message.envVars?.length) {
            obj.envVars = message.envVars.map((e) => EnvVar.toJSON(e));
        }
        if (message.agent !== "") {
            obj.agent = message.agent;
        }
        if (message.timestamps !== undefined) {
            obj.timestamps = Timestamps.toJSON(message.timestamps);
        }
        if (message.dockerfile !== "") {
            obj.dockerfile = message.dockerfile;
        }
        return obj;
    },
};
function createBaseListRuntimesRequest() {
    return {};
}
export const ListRuntimesRequest = {
    encode(_, writer = new BinaryWriter()) {
        return writer;
    },
    decode(input, length) {
        const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
        const end = length === undefined ? reader.len : reader.pos + length;
        const message = createBaseListRuntimesRequest();
        while (reader.pos < end) {
            const tag = reader.uint32();
            switch (tag >>> 3) {
            }
            if ((tag & 7) === 4 || tag === 0) {
                break;
            }
            reader.skip(tag & 7);
        }
        return message;
    },
    fromJSON(_) {
        return {};
    },
    toJSON(_) {
        const obj = {};
        return obj;
    },
};
function createBaseListRuntimesResponse() {
    return { runtimes: [] };
}
export const ListRuntimesResponse = {
    encode(message, writer = new BinaryWriter()) {
        for (const v of message.runtimes) {
            Runtime.encode(v, writer.uint32(10).fork()).join();
        }
        return writer;
    },
    decode(input, length) {
        const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
        const end = length === undefined ? reader.len : reader.pos + length;
        const message = createBaseListRuntimesResponse();
        while (reader.pos < end) {
            const tag = reader.uint32();
            switch (tag >>> 3) {
                case 1: {
                    if (tag !== 10) {
                        break;
                    }
                    message.runtimes.push(Runtime.decode(reader, reader.uint32()));
                    continue;
                }
            }
            if ((tag & 7) === 4 || tag === 0) {
                break;
            }
            reader.skip(tag & 7);
        }
        return message;
    },
    fromJSON(object) {
        return {
            runtimes: globalThis.Array.isArray(object?.runtimes) ? object.runtimes.map((e) => Runtime.fromJSON(e)) : [],
        };
    },
    toJSON(message) {
        const obj = {};
        if (message.runtimes?.length) {
            obj.runtimes = message.runtimes.map((e) => Runtime.toJSON(e));
        }
        return obj;
    },
};
function createBaseGetRuntimeRequest() {
    return { id: "" };
}
export const GetRuntimeRequest = {
    encode(message, writer = new BinaryWriter()) {
        if (message.id !== "") {
            writer.uint32(10).string(message.id);
        }
        return writer;
    },
    decode(input, length) {
        const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
        const end = length === undefined ? reader.len : reader.pos + length;
        const message = createBaseGetRuntimeRequest();
        while (reader.pos < end) {
            const tag = reader.uint32();
            switch (tag >>> 3) {
                case 1: {
                    if (tag !== 10) {
                        break;
                    }
                    message.id = reader.string();
                    continue;
                }
            }
            if ((tag & 7) === 4 || tag === 0) {
                break;
            }
            reader.skip(tag & 7);
        }
        return message;
    },
    fromJSON(object) {
        return { id: isSet(object.id) ? globalThis.String(object.id) : "" };
    },
    toJSON(message) {
        const obj = {};
        if (message.id !== "") {
            obj.id = message.id;
        }
        return obj;
    },
};
function createBaseCreateRuntimeRequest() {
    return {
        name: "",
        description: "",
        providerId: "",
        allowedModels: [],
        defaultModel: "",
        containerImage: "",
        installCommand: "",
        envVars: [],
        dockerfile: "",
    };
}
export const CreateRuntimeRequest = {
    encode(message, writer = new BinaryWriter()) {
        if (message.name !== "") {
            writer.uint32(10).string(message.name);
        }
        if (message.description !== "") {
            writer.uint32(18).string(message.description);
        }
        if (message.providerId !== "") {
            writer.uint32(26).string(message.providerId);
        }
        for (const v of message.allowedModels) {
            writer.uint32(34).string(v);
        }
        if (message.defaultModel !== "") {
            writer.uint32(42).string(message.defaultModel);
        }
        if (message.containerImage !== "") {
            writer.uint32(50).string(message.containerImage);
        }
        if (message.installCommand !== "") {
            writer.uint32(58).string(message.installCommand);
        }
        for (const v of message.envVars) {
            EnvVar.encode(v, writer.uint32(66).fork()).join();
        }
        if (message.dockerfile !== "") {
            writer.uint32(74).string(message.dockerfile);
        }
        return writer;
    },
    decode(input, length) {
        const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
        const end = length === undefined ? reader.len : reader.pos + length;
        const message = createBaseCreateRuntimeRequest();
        while (reader.pos < end) {
            const tag = reader.uint32();
            switch (tag >>> 3) {
                case 1: {
                    if (tag !== 10) {
                        break;
                    }
                    message.name = reader.string();
                    continue;
                }
                case 2: {
                    if (tag !== 18) {
                        break;
                    }
                    message.description = reader.string();
                    continue;
                }
                case 3: {
                    if (tag !== 26) {
                        break;
                    }
                    message.providerId = reader.string();
                    continue;
                }
                case 4: {
                    if (tag !== 34) {
                        break;
                    }
                    message.allowedModels.push(reader.string());
                    continue;
                }
                case 5: {
                    if (tag !== 42) {
                        break;
                    }
                    message.defaultModel = reader.string();
                    continue;
                }
                case 6: {
                    if (tag !== 50) {
                        break;
                    }
                    message.containerImage = reader.string();
                    continue;
                }
                case 7: {
                    if (tag !== 58) {
                        break;
                    }
                    message.installCommand = reader.string();
                    continue;
                }
                case 8: {
                    if (tag !== 66) {
                        break;
                    }
                    message.envVars.push(EnvVar.decode(reader, reader.uint32()));
                    continue;
                }
                case 9: {
                    if (tag !== 74) {
                        break;
                    }
                    message.dockerfile = reader.string();
                    continue;
                }
            }
            if ((tag & 7) === 4 || tag === 0) {
                break;
            }
            reader.skip(tag & 7);
        }
        return message;
    },
    fromJSON(object) {
        return {
            name: isSet(object.name) ? globalThis.String(object.name) : "",
            description: isSet(object.description) ? globalThis.String(object.description) : "",
            providerId: isSet(object.providerId)
                ? globalThis.String(object.providerId)
                : isSet(object.provider_id)
                    ? globalThis.String(object.provider_id)
                    : "",
            allowedModels: globalThis.Array.isArray(object?.allowedModels)
                ? object.allowedModels.map((e) => globalThis.String(e))
                : globalThis.Array.isArray(object?.allowed_models)
                    ? object.allowed_models.map((e) => globalThis.String(e))
                    : [],
            defaultModel: isSet(object.defaultModel)
                ? globalThis.String(object.defaultModel)
                : isSet(object.default_model)
                    ? globalThis.String(object.default_model)
                    : "",
            containerImage: isSet(object.containerImage)
                ? globalThis.String(object.containerImage)
                : isSet(object.container_image)
                    ? globalThis.String(object.container_image)
                    : "",
            installCommand: isSet(object.installCommand)
                ? globalThis.String(object.installCommand)
                : isSet(object.install_command)
                    ? globalThis.String(object.install_command)
                    : "",
            envVars: globalThis.Array.isArray(object?.envVars)
                ? object.envVars.map((e) => EnvVar.fromJSON(e))
                : globalThis.Array.isArray(object?.env_vars)
                    ? object.env_vars.map((e) => EnvVar.fromJSON(e))
                    : [],
            dockerfile: isSet(object.dockerfile) ? globalThis.String(object.dockerfile) : "",
        };
    },
    toJSON(message) {
        const obj = {};
        if (message.name !== "") {
            obj.name = message.name;
        }
        if (message.description !== "") {
            obj.description = message.description;
        }
        if (message.providerId !== "") {
            obj.providerId = message.providerId;
        }
        if (message.allowedModels?.length) {
            obj.allowedModels = message.allowedModels;
        }
        if (message.defaultModel !== "") {
            obj.defaultModel = message.defaultModel;
        }
        if (message.containerImage !== "") {
            obj.containerImage = message.containerImage;
        }
        if (message.installCommand !== "") {
            obj.installCommand = message.installCommand;
        }
        if (message.envVars?.length) {
            obj.envVars = message.envVars.map((e) => EnvVar.toJSON(e));
        }
        if (message.dockerfile !== "") {
            obj.dockerfile = message.dockerfile;
        }
        return obj;
    },
};
function createBaseUpdateRuntimeRequest() {
    return {
        id: "",
        name: undefined,
        description: undefined,
        providerId: undefined,
        allowedModels: [],
        defaultModel: undefined,
        containerImage: undefined,
        installCommand: undefined,
        envVars: [],
        dockerfile: undefined,
    };
}
export const UpdateRuntimeRequest = {
    encode(message, writer = new BinaryWriter()) {
        if (message.id !== "") {
            writer.uint32(10).string(message.id);
        }
        if (message.name !== undefined) {
            writer.uint32(18).string(message.name);
        }
        if (message.description !== undefined) {
            writer.uint32(26).string(message.description);
        }
        if (message.providerId !== undefined) {
            writer.uint32(34).string(message.providerId);
        }
        for (const v of message.allowedModels) {
            writer.uint32(42).string(v);
        }
        if (message.defaultModel !== undefined) {
            writer.uint32(50).string(message.defaultModel);
        }
        if (message.containerImage !== undefined) {
            writer.uint32(58).string(message.containerImage);
        }
        if (message.installCommand !== undefined) {
            writer.uint32(66).string(message.installCommand);
        }
        for (const v of message.envVars) {
            EnvVar.encode(v, writer.uint32(74).fork()).join();
        }
        if (message.dockerfile !== undefined) {
            writer.uint32(82).string(message.dockerfile);
        }
        return writer;
    },
    decode(input, length) {
        const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
        const end = length === undefined ? reader.len : reader.pos + length;
        const message = createBaseUpdateRuntimeRequest();
        while (reader.pos < end) {
            const tag = reader.uint32();
            switch (tag >>> 3) {
                case 1: {
                    if (tag !== 10) {
                        break;
                    }
                    message.id = reader.string();
                    continue;
                }
                case 2: {
                    if (tag !== 18) {
                        break;
                    }
                    message.name = reader.string();
                    continue;
                }
                case 3: {
                    if (tag !== 26) {
                        break;
                    }
                    message.description = reader.string();
                    continue;
                }
                case 4: {
                    if (tag !== 34) {
                        break;
                    }
                    message.providerId = reader.string();
                    continue;
                }
                case 5: {
                    if (tag !== 42) {
                        break;
                    }
                    message.allowedModels.push(reader.string());
                    continue;
                }
                case 6: {
                    if (tag !== 50) {
                        break;
                    }
                    message.defaultModel = reader.string();
                    continue;
                }
                case 7: {
                    if (tag !== 58) {
                        break;
                    }
                    message.containerImage = reader.string();
                    continue;
                }
                case 8: {
                    if (tag !== 66) {
                        break;
                    }
                    message.installCommand = reader.string();
                    continue;
                }
                case 9: {
                    if (tag !== 74) {
                        break;
                    }
                    message.envVars.push(EnvVar.decode(reader, reader.uint32()));
                    continue;
                }
                case 10: {
                    if (tag !== 82) {
                        break;
                    }
                    message.dockerfile = reader.string();
                    continue;
                }
            }
            if ((tag & 7) === 4 || tag === 0) {
                break;
            }
            reader.skip(tag & 7);
        }
        return message;
    },
    fromJSON(object) {
        return {
            id: isSet(object.id) ? globalThis.String(object.id) : "",
            name: isSet(object.name) ? globalThis.String(object.name) : undefined,
            description: isSet(object.description) ? globalThis.String(object.description) : undefined,
            providerId: isSet(object.providerId)
                ? globalThis.String(object.providerId)
                : isSet(object.provider_id)
                    ? globalThis.String(object.provider_id)
                    : undefined,
            allowedModels: globalThis.Array.isArray(object?.allowedModels)
                ? object.allowedModels.map((e) => globalThis.String(e))
                : globalThis.Array.isArray(object?.allowed_models)
                    ? object.allowed_models.map((e) => globalThis.String(e))
                    : [],
            defaultModel: isSet(object.defaultModel)
                ? globalThis.String(object.defaultModel)
                : isSet(object.default_model)
                    ? globalThis.String(object.default_model)
                    : undefined,
            containerImage: isSet(object.containerImage)
                ? globalThis.String(object.containerImage)
                : isSet(object.container_image)
                    ? globalThis.String(object.container_image)
                    : undefined,
            installCommand: isSet(object.installCommand)
                ? globalThis.String(object.installCommand)
                : isSet(object.install_command)
                    ? globalThis.String(object.install_command)
                    : undefined,
            envVars: globalThis.Array.isArray(object?.envVars)
                ? object.envVars.map((e) => EnvVar.fromJSON(e))
                : globalThis.Array.isArray(object?.env_vars)
                    ? object.env_vars.map((e) => EnvVar.fromJSON(e))
                    : [],
            dockerfile: isSet(object.dockerfile) ? globalThis.String(object.dockerfile) : undefined,
        };
    },
    toJSON(message) {
        const obj = {};
        if (message.id !== "") {
            obj.id = message.id;
        }
        if (message.name !== undefined) {
            obj.name = message.name;
        }
        if (message.description !== undefined) {
            obj.description = message.description;
        }
        if (message.providerId !== undefined) {
            obj.providerId = message.providerId;
        }
        if (message.allowedModels?.length) {
            obj.allowedModels = message.allowedModels;
        }
        if (message.defaultModel !== undefined) {
            obj.defaultModel = message.defaultModel;
        }
        if (message.containerImage !== undefined) {
            obj.containerImage = message.containerImage;
        }
        if (message.installCommand !== undefined) {
            obj.installCommand = message.installCommand;
        }
        if (message.envVars?.length) {
            obj.envVars = message.envVars.map((e) => EnvVar.toJSON(e));
        }
        if (message.dockerfile !== undefined) {
            obj.dockerfile = message.dockerfile;
        }
        return obj;
    },
};
function createBaseDeleteRuntimeRequest() {
    return { id: "" };
}
export const DeleteRuntimeRequest = {
    encode(message, writer = new BinaryWriter()) {
        if (message.id !== "") {
            writer.uint32(10).string(message.id);
        }
        return writer;
    },
    decode(input, length) {
        const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
        const end = length === undefined ? reader.len : reader.pos + length;
        const message = createBaseDeleteRuntimeRequest();
        while (reader.pos < end) {
            const tag = reader.uint32();
            switch (tag >>> 3) {
                case 1: {
                    if (tag !== 10) {
                        break;
                    }
                    message.id = reader.string();
                    continue;
                }
            }
            if ((tag & 7) === 4 || tag === 0) {
                break;
            }
            reader.skip(tag & 7);
        }
        return message;
    },
    fromJSON(object) {
        return { id: isSet(object.id) ? globalThis.String(object.id) : "" };
    },
    toJSON(message) {
        const obj = {};
        if (message.id !== "") {
            obj.id = message.id;
        }
        return obj;
    },
};
export const RuntimeServiceService = {
    listRuntimes: {
        path: "/core.v1.RuntimeService/ListRuntimes",
        requestStream: false,
        responseStream: false,
        requestSerialize: (value) => Buffer.from(ListRuntimesRequest.encode(value).finish()),
        requestDeserialize: (value) => ListRuntimesRequest.decode(value),
        responseSerialize: (value) => Buffer.from(ListRuntimesResponse.encode(value).finish()),
        responseDeserialize: (value) => ListRuntimesResponse.decode(value),
    },
    getRuntime: {
        path: "/core.v1.RuntimeService/GetRuntime",
        requestStream: false,
        responseStream: false,
        requestSerialize: (value) => Buffer.from(GetRuntimeRequest.encode(value).finish()),
        requestDeserialize: (value) => GetRuntimeRequest.decode(value),
        responseSerialize: (value) => Buffer.from(Runtime.encode(value).finish()),
        responseDeserialize: (value) => Runtime.decode(value),
    },
    createRuntime: {
        path: "/core.v1.RuntimeService/CreateRuntime",
        requestStream: false,
        responseStream: false,
        requestSerialize: (value) => Buffer.from(CreateRuntimeRequest.encode(value).finish()),
        requestDeserialize: (value) => CreateRuntimeRequest.decode(value),
        responseSerialize: (value) => Buffer.from(Runtime.encode(value).finish()),
        responseDeserialize: (value) => Runtime.decode(value),
    },
    updateRuntime: {
        path: "/core.v1.RuntimeService/UpdateRuntime",
        requestStream: false,
        responseStream: false,
        requestSerialize: (value) => Buffer.from(UpdateRuntimeRequest.encode(value).finish()),
        requestDeserialize: (value) => UpdateRuntimeRequest.decode(value),
        responseSerialize: (value) => Buffer.from(Runtime.encode(value).finish()),
        responseDeserialize: (value) => Runtime.decode(value),
    },
    deleteRuntime: {
        path: "/core.v1.RuntimeService/DeleteRuntime",
        requestStream: false,
        responseStream: false,
        requestSerialize: (value) => Buffer.from(DeleteRuntimeRequest.encode(value).finish()),
        requestDeserialize: (value) => DeleteRuntimeRequest.decode(value),
        responseSerialize: (value) => Buffer.from(DeleteResponse.encode(value).finish()),
        responseDeserialize: (value) => DeleteResponse.decode(value),
    },
};
export const RuntimeServiceClient = makeGenericClientConstructor(RuntimeServiceService, "core.v1.RuntimeService");
function isSet(value) {
    return value !== null && value !== undefined;
}
//# sourceMappingURL=runtime.js.map